...
| `MARKET_TTL_TECHNICALS_SECONDS` | TTL for `/v1/market/technicals/{symbol}` — trend, golden/death cross, RSI, MACD and the 52-week band, all derived from the cached history rather than fetched. Redis only; floored at `60` however low it is set. | `3600` (1h) |
